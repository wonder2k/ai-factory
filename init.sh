...
# ==============================================================================
# 🚀 Hermes AI Factory - 终极无瑕完全体 (全局插件固化 + DeepSeek 代理 + Karpathy原则)
# ==============================================================================
...
echo "===================================================="
echo "🌱 开始从零释放【全局插件固化版】AI Factory 全套配置文件..."
...
# D. 🌟 动态点火：容器启动时完成 fcc 密钥注入、后台代理拉起、10大官方插件静默收割、并直接修改全局 JSON 强行激活插件
CMD ["sh", "-c", "\
...
echo '⚙️  5. 🌟 正在将 10 大插件全局写入 settings.json 固化激活状态...' && \
mkdir -p ~/.claude && \
echo '{\"mcpServers\":{},\"enabledPlugins\":[\"claude-code-setup\",\"claude-md-management\",\"code-review\",\"code-simplifier\",\"context7\",\"feature-dev\",\"frontend-design\",\"playwright\",\"superpowers\",\"security-guidance\"]}' > ~/.claude/settings.json && \
echo '🧠 6. 原地唤醒 Telegram 总控大脑...' && \
...
# 5. 写入 hermes-server.js (★ 剥离失败的 --plugin 参数，回归纯净的原生调用)
                ...
                if (data.ok && data.result.length > 0) {
                    for (const update of data.result) {
                        ...
                        if (update.message && update.message.text) {
                            ...
                            setTimeout(() => {
                                // 🌟 插件现在由全局 settings.json 托管，直接原生起飞，再也不需要任何附加参数
...
console.log("🚀 Hermes 总控大脑【全局固化版】已点火，正在丝滑监听 Telegram 消息...");
pollUpdates();
EOF
...
echo "===================================================="
echo "🎉 恭喜！完美全局固化激活插件的完全体脚手架已成功吐出！"
echo "👉 接下来请直接运行: docker compose up -d --build"
echo "===================================================="
